...
def dfs(loc):
    ...
    for i in directions:
        dr, dc = directions[i]
        if c+dc not in grid[r+dr]:
            status = k(i)
            grid[r+dr][c+dc] = status
...
while True:
    ...
    if grid[r][c] == 1:
        for i in directions:
            dr, dc = directions[i]
            if (r+dr,c+dc) not in explored: 
                bfs.append((dr+r, dc+c, d+1))
# def dfs_2(r, c):
#     if grid[r][c] == 2:
#         return 0
#     if grid[r][c] == 0:
