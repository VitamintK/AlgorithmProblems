...
while len(pq) > 0:
    ...
    if (fr, cur) in explored:
        continue
    explored.add((fr, cur))
    for ind, robot in enumerate(cur):
        for k in keys:
            if k in collected:
                continue
            if k not in edges[robot] or len(edges[robot][k][1] - collected) > 0:
            ...
            if (frozenset(c), k) in explored:
                continue
            heappush(pq, (d + edges[robot][k][0], c, tuple(k if x == robot else x for x in cur)))
